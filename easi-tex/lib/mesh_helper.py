...
def init_mesh(input_path, cache_path, device):
    ...
    # Load the edge mesh
    edge_mesh = load_objs_as_meshes([input_path], device=device)

    # make sure mesh center is at origin
    mesh_center = (torch.max(mesh.verts_packed(), dim=0)[0] + torch.min(mesh.verts_packed(), dim=0)[0]) / 2
    mesh = apply_offsets_to_mesh(mesh, -mesh_center)
    edge_mesh = apply_offsets_to_mesh(edge_mesh, -mesh_center)
